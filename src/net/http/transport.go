...
var errCloseConns = errors.New("http: CloseConnections called")
...
// currently in use.
func (t *Transport) CloseConnections(f func(conn net.Conn, idle bool) bool) {
	t.nextProtoOnce.Do(t.onceSetNextProtoDefaults)
	t.idleMu.Lock()
	m := t.idleConn
	t.idleConn = nil
	...
	for _, conns := range m {
		for _, pconn := range conns {
			if f(pconn.conn, true) {
				pconn.close(errCloseConns)
			}
		}
	}
	if t2 := t.h2transport; t2 != nil {
		if p, ok := t2.connPool().(*http2clientConnPool); ok {
			...
			for _, vv := range p.conns {
				for _, cc := range vv {
					cc.mu.Lock()
					if f(cc.tconn, len(cc.streams) == 0) {
						cc.closed = true
					}
					cc.mu.Unlock()
					if cc.closed {
